...
export default async function CreditorsAgeingPage() {
  ...
  const report = await getAgeingReport('creditors')
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Creditors Ageing Report</h1>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-3xl font-bold text-green-900">RM {report?.summary?.total?.toLocaleString()}</p>
                </div>
              </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Ageing by Vendor</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {report?.details?.map((vendor: any, idx: number) => (
                    <tr key={idx} className="border-b hover:bg-gray-50">
                      <td className="px-4 py-3 font-medium">{vendor.vendor}</td>
                      <td className="px-4 py-3 text-right">{vendor.current?.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right">{vendor.days1to30?.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right">{vendor.days31to60?.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right">{vendor.days61to90?.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right">{vendor.over90?.toLocaleString()}</td>
                      <td className="px-4 py-3 text-right font-bold">{vendor.total?.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
